...
$url = new moodle_url('/local/backup_course/create/layouts/modedit.php');
...
echo '<link href="' . $CFG->wwwroot . '/local/backup_course/css/style.css" rel="stylesheet" type="text/css" />' .
    //'<script src="'.$CFG->wwwroot.'/local/backup_course/create/js/up_create.js"></script>'.
...
if (!empty($add)) {
    ...
    navigation_node::override_active_url(new moodle_url('/local/backup_course/create/layouts/modedit.php', array('update' => $update, 'return' => 1)));
...
if ($mform->is_cancelled()) {
    ...
    //echo '<pre>$fromform: '; print_r($fromform); echo '</pre>';
    //var_dump($fromform->showgradingmanagement);

    if (!empty($fromform->update)) {
        list($cm, $fromform) = update_moduleinfo($cm, $fromform, $course, $mform);
    } else if (!empty($fromform->add)) {
        include '../../util.php';
        $datos1 = addslashes(json_encode($fromform));
        $datos2 = addslashes(json_encode($mform));
        //echo '<pre>$fromform: '; print_r($fromform); echo '</pre>'; die();
        if (isset($fromform->submitbutton)) { //le da a guardar y ver
            $fromform = add_moduleinfo($fromform, $course, $mform);
            $url = new moodle_url("/mod/$module->name/view.php", array('id' => $fromform->coursemodule, 'forceview' => 1));
            if (empty($fromform->showgradingmanagement)) { //guardar y ver
                redirect($url);
            } else {
            ...
            if (isset($fromform->assessed)) { //se puede configurar la calificación
                if (empty($fromform->assessed)) { //no se configuró la calificación
                    $fromform = add_moduleinfo($fromform, $course, $mform);
                    $url = new moodle_url("/mod/$module->name/view.php", array('id' => $fromform->coursemodule, 'forceview' => 1));

                    echo html_writer::script('r_url.list_actualizar_create_save("' . $CFG->wwwroot . '",' . $course->id . ',"' . $url . '","' . $datos1 . '","' . $datos2 . '");');
                } else { //trae calificación
                    echo html_writer::script('r_url.alert_no_save();');
                    die();
                }
            } else { //actividad no calificable
                $fromform = add_moduleinfo($fromform, $course, $mform);
                $url = new moodle_url("/mod/$module->name/view.php", array('id' => $fromform->coursemodule, 'forceview' => 1));
                echo html_writer::script('r_url.list_actualizar_create_save("' . $CFG->wwwroot . '",' . $course->id . ',"' . $url . '","' . $datos1 . '","' . $datos2 . '");');
            }
