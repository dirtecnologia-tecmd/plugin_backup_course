...
$url = new moodle_url('/local/backup_course/bank/mod.php');
...
if (!empty($add)) {
    ...
    redirect("$CFG->wwwroot/local/backup_course/bank/modedit.php?add=$add&type=$type&course=$id&section=$section&return=$returntomod&sr=$sectionreturn");

} else if (!empty($update)) {
    $cm = get_coursemodule_from_id('', $update, 0, true, MUST_EXIST);
    $returntomod = optional_param('return', 0, PARAM_BOOL);
    redirect("$CFG->wwwroot/local/backup_course/bank/modedit.php?update=$update&return=$returntomod&sr=$sectionreturn");
    ...
    if (!$confirm or !confirm_sesskey()) {
        ...
        $formcontinue = new single_button(new moodle_url("$CFG->wwwroot/local/backup_course/bank/mod.php", $optionsyes), get_string('yes'));
