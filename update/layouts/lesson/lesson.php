...
$url = new moodle_url('/local/backup_course/update/layouts/lesson/lesson.php', array('id'=>$id,'action'=>$action));
$PAGE->set_url($url);
...
switch ($action) {
    ...
    echo '<link href="'.$CFG->wwwroot .'/local/backup_course/css/style.css" rel="stylesheet" type="text/css" />'.
        ...
        $thispage = $lesson->load_page($pageid);
        $thispage->delete();
        $redi = "$CFG->wwwroot/local/backup_course/update/layouts/lesson/edit.php?id=$cm->id";
        $datos = $lesson->properties();
        echo '<div id="snackbar"></div>';
        echo html_writer::script('SLog.confir_nodos_actu('.$course->id.', '.$cm->id.', "'.$cm->modname.'", '.$USER->id.',"'.addslashes(json_encode($datos)).'", "'.null.'", "../../../", "'.$redi.'");'     );
        die();
        //redirect("$CFG->wwwroot/local/backup_course/update/layouts/lesson/edit.php?id=$cm->id");
        break;
    case 'moveit':
    ...
    case 'duplicate':
            ...
            redirect(new moodle_url('/local/backup_course/update/layouts/lesson/edit.php', array('id' => $cm->id)));
        break;
    default:
