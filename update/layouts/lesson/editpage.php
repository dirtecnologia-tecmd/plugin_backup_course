...
require_once($CFG->dirroot.'/mod/lesson/editpage_form.php');
...
if (empty($returnto)) {
    $returnto = new moodle_url('/local/backup_course/update/layouts/lesson/edit.php', array('id' => $id));
...
$PAGE->set_url('/local/backup_course/update/layouts/lesson/editpage.php', array('pageid'=>$pageid, 'id'=>$id, 'qtype'=>$qtype));
...
if ($edit) {
    ...
    $mform->set_data($data);
    $PAGE->navbar->add(get_string('edit'), new moodle_url('/local/backup_course/update/layouts/lesson/edit.php', array('id'=>$id)));
...
if ($data = $mform->get_data()) {
    ...
    if ($edit) {
        ...
        $new_create = 1; //crear page
        $editpage = lesson_page::create($data, $lesson, $context, $PAGE->course->maxbytes);
    }
    echo '<div id="snackbar"></div>';
    echo html_writer::script('SLog.confir_nodos_actu('.$course->id.', '.$cm->id.', "'.$cm->modname.'", '.$USER->id.',"'.addslashes(json_encode($datos)).'", "'.null.'", "../../../", "'.$returnto.'", '.$new_create.');'     );
        die();
    redirect($returnto);
}
