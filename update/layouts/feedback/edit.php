...
require_once($CFG->dirroot.'/mod/feedback/edit_form.php');
...
$url = new moodle_url('/local/backup_course/update/layouts/feedback/edit.php', array('id'=>$id, 'do_show'=>$do_show));
...
if ($deleteitem) {
    ...
    if (isset($items[$deleteitem])) {
        ...
        $dat = $items[$deleteitem];
        echo '<div id="snackbar"></div>';
        echo html_writer::script('SLog.confir_nodos_actu('.$course->id.', '.$cm->id.', "'.$cm->modname.'", '.$USER->id.',"'.addslashes(json_encode($dat)).'", '.$cm->sectionnum.', "../../../", "'.$url.'");'     );
        die;
    }
    //redirect($url);
}
...
$use_template_form = new feedback_edit_use_template_form($CFG->dirroot.'/mod/feedback/use_templ.php', array('course' => $course, 'id' => $id));

//Print the page header.
$strfeedbacks = get_string('modulenameplural', 'feedback');
$strfeedback  = get_string('modulename', 'feedback');

$PAGE->set_url('/local/backup_course/update/layouts/feedback/edit.php', array('id'=>$cm->id, 'do_show'=>$do_show));
...
/// print the tabs
require($CFG->dirroot.'/mod/feedback/tabs.php');
...
if ($do_show == 'edit') {
    ...
    $select = new single_select(new moodle_url('/local/backup_course/update/layouts/feedback/edit_item.php',
