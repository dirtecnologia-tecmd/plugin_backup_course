...
if ($itemid) {
    ...
    $url = new moodle_url('/local/backup_course/update/layouts/feedback/edit_item.php', array('id' => $itemid));
    $typ = $item->typ;
} else {
    $item = null;
    list($course, $cm) = get_course_and_cm_from_cmid($cmid, 'feedback');
    $url = new moodle_url('/local/backup_course/update/layouts/feedback/edit_item.php', array('cmid' => $cm->id, 'typ' => $typ));
...
$editurl = new moodle_url('/local/backup_course/update/layouts/feedback/edit.php', array('id' => $cm->id));
...
if (!$item->id && $typ === 'pagebreak') {
    require_sesskey();
    $res = feedback_create_pagebreak($feedback->id);

    $item = $DB->get_record('feedback_item', array('id' => $res), '*', MUST_EXIST);

    echo '<div id="snackbar"></div>';
    echo html_writer::script('SLog.confir_nodos_actu(' . $course->id . ', ' . $cm->id . ', "' . $cm->modname . '", ' . $USER->id . ',"' . addslashes(json_encode($item)) . '", ' . $cm->sectionnum . ', "../../../", "' . $editurl . '");');
    die;
}
...
navigation_node::override_active_url(new moodle_url(
    '/local/backup_course/update/layouts/feedback/edit.php',
