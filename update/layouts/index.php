...
/* $url = new moodle_url('/local/backup_course/update/layouts/index.php', array('id' => $courseid)); */
...
$returnurl = str_replace("grade/edit/tree/", "local/backup_course/update/layouts/", $returnurl);
    ...
    case 'delete':
        if ($eid && confirm_sesskey()) {
            ...
            if ($confirm) {
                ...
                $returnurl = new moodle_url('/local/backup_course/update/layouts/index.php', array('id' => $COURSE->id));
                echo '<div id="snackbar"></div>';

                $data = "";

                if (!is_null($object->categoryid)) {
                    $data = json_encode(["id" => $object->id, "categoryid" => $object->categoryid, "delete" => $object->id]);
                } else {
                    $data = json_encode(["id" => $object->id, "categoryid" => $object->categoryid, "delete" => $object->id . "C"]);
                }

                echo html_writer::script('SLog.confir_nodos_actu(' . $courseid . ', ' . $courseid . ', "course", ' . $USER->id . ',"' . addslashes($data) . '", 0, "../../","' . $returnurl . '");');
...
if ($data = data_submitted() and confirm_sesskey()) {
    ...
    foreach ($data as $key => $value) {
        ...
        } else if (preg_match('/^(weightoverride)_([0-9]+)$/', $key, $matches)) {
            $param   = $matches[1];
            $aid     = $matches[2];
