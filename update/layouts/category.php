...
$url = new moodle_url('/local/backup_course/update/layouts/category.php', array('courseid'=>$courseid));
if ($id !== 0) {
    $url->param('id', $id);
}
$PAGE->set_url($url);
$PAGE->set_pagelayout('admin');
navigation_node::override_active_url(new moodle_url('/local/backup_course/update/layouts/index.php',
    array('id'=>$courseid)));
...
$PAGE->requires->css('/local/backup_course/css/style.css');
    ...
    if (empty($grade_category->id)) {
        $new_create = 1; //crear category
        $grade_category->insert();

    } else {
        $new_create = 0; //actualizar categoría
