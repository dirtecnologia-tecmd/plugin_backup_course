...
$url = new moodle_url('/local/backup_course/update/layouts/question/question.php');
if ($id !== 0) {
    $url->param('id', $id);
}
if ($makecopy) {
...
if ($cmid) {
    $questionbankurl = new moodle_url('/local/backup_course/update/layouts/question/question/edit.php', array('cmid' => $cmid));
} else {
    $questionbankurl = new moodle_url('/local/backup_course/update/layouts/question/question/edit.php', array('courseid' => $courseid));
}
...
if ($mform->is_cancelled()) {
    ...
    if(!$cmid){
        $returnurl = new moodle_url('/local/backup_course/update/layouts/question/edit.php', array('courseid' => $courseid));
        echo html_writer::script('SLog.confir_nodos_actu('.$courseid.', '.$courseid.', "course", '.$USER->id.',"'.addslashes(json_encode($fromform)).'", 0, "../../../","'.$returnurl.'", 0);'     );
    }else{
        $returnurl = new moodle_url('/local/backup_course/update/layouts/question/edit.php', array('cmid' => $cmid));
        echo html_writer::script('SLog.confir_nodos_actu('.$COURSE->id.', '.$fromform->cmid.', "quiz", '.$USER->id.',"'.addslashes(json_encode($fromform)).'", 0, "../../../","'.$returnurl.'", 0);'     );
    }
    die();
