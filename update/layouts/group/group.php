...
require_once('../../../../../group/group_form.php');
...
navigation_node::override_active_url(new moodle_url('/local/backup_course/update/layouts/group/index.php', array('id' => $course->id)));

$returnurl = $CFG->wwwroot.'/local/backup_course/update/layouts/group/index.php?id='.$course->id.'&group='.$id;
...
if ($editform->is_cancelled()) {
    ...
    echo html_writer::script('SLog.confir_nodos_actu('.$courseid.', '.$courseid.', "course", '.$USER->id.',"'.addslashes(json_encode($data)).'", 0, "../../../","'.$returnurl.'", '.$new_create.');'     );
    //redirect($returnurl);
}
...
$PAGE->navbar->add($strgroups, new moodle_url('/local/backup_course/update/layouts/group/index.php', array('id'=>$courseid)));
