...
if (!empty($shareform)) {
    ...
    if (!$confirmed) {
        ...
        echo $output->header();
        echo $output->confirm(get_string('manageactionshareconfirm', 'core_grading', ($definition->name)),
...
if (!empty($method)) {
    ...
    if ($controller->is_form_defined()) {
        ...
        $pickurl = new moodle_url('/local/backup_course/update/layouts/rubric/pick.php', array('targetid' => $controller->get_areaid(),'cmid' =>$cm->id));
    ...
    if ($controller->is_form_defined()) {
        ...
        echo $output->heading(($definition->name) . ' ' . $tag, 3, 'definition-name');
