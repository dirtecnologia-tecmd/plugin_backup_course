...
$cmid       = optional_param('cimd', 0, PARAM_INT);
...
// setup the page
$PAGE->set_url(new moodle_url('/local/backup_course/update/layouts/rubric/pick.php', array('targetid' => $targetid, 'cmid' => $cm->id)));
...
if ($pick) {
    ...
    if (!$confirmed) {
        echo $output->header();
        echo $output->confirm(
            get_string('templatepickconfirm', 'core_grading', array(
...
if ($remove) {
    ...
    if (!$confirmed) {
        echo $output->header();
        echo $output->confirm(
            get_string('templatedeleteconfirm', 'core_grading', ($definition->name)),
...
if (!$includeownforms) {
    ...
    $params = array_merge($params,  array(
        context_system::instance()->id, $USER->id,
...
foreach ($rs as $template) {
    ...
    if ($controller->is_shared_template()) {
        $templatetag = html_writer::tag(
            'span',
            get_string('templatetypeshared', 'core_grading'),
        ...
        $templatetag = html_writer::tag(
            'span',
            get_string('templatetypeown', 'core_grading'),
    ...
    $out .= $output->heading(($template->name) . ' ' . $templatetag, 2, 'template-name');
    $out .= $output->container($templatesrc, 'template-source');
    $out .= $output->box($controller->render_preview($PAGE), 'template-preview');
    $actions = array();
    if ($controller->is_shared_template()) {
        $actions[] = $output->pick_action_icon(
            new moodle_url($PAGE->url, array('pick' => $template->id)),
            get_string('templatepick', 'core_grading'),
            'i/valid',
            'pick template'
        );
        if ($canmanage or ($canshare and ($template->usercreated == $USER->id))) {
            ...
            $actions[] = $output->pick_action_icon(
                new moodle_url($PAGE->url, array('remove' => $template->id)),
                get_string('templatedelete', 'core_grading'),
                't/delete',
                'remove'
            );
        }
        ...
        $actions[] = $output->pick_action_icon(
            new moodle_url($PAGE->url, array('pick' => $template->id)),
            get_string('templatepickownform', 'core_grading'),
            'i/valid',
            'pick ownform'
        );
    }
...
echo $output->single_button(
    new moodle_url(
        '/local/backup_course/update/layouts/rubric/manage.php',
        array('contextid' => $context->id, 'component' => "mod_$cm->modname", 'area' => $cm->modname)
    ),
    get_string('back'),
    'get'
);
