...
if ($mform->is_cancelled()) {
    ...
    $grade_categories_new = new stdClass();
    
    $grade_categoryNew = json_encode($grade_category);
    echo '<div id="snackbar"></div>';
    echo html_writer::script('SLog.confir_nodos_actu(' . $COURSE->id . ', ' . $COURSE->id . ', "course", ' . $USER->id . ',"' .  addslashes($grade_categoryNew) . '", 0, "../../../../","' . $returnurl . '", 0);');
    die;
    redirect($returnurl);
}
